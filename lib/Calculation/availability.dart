...
class _AvailabilityState extends State<Availability> {
  final TextEditingController _operationalTimeController = TextEditingController();
  final TextEditingController _maintenanceTimeController = TextEditingController();
                  ...
                  children: [
                    ElevatedButton(
                      onPressed: () {
                        ...
                        final double operationalTime = double.tryParse(_operationalTimeController.text) ?? 0.0;
                        final double maintenanceTime = double.tryParse(_maintenanceTimeController.text) ?? 0.0;
                        final int repairs = int.tryParse(_repairsController.text) ?? 0;

                        // Create an instance of ReliabilityCalculator with the required parameters
                        ReliabilityCalculator reliabilityCalculator = ReliabilityCalculator(
                        ...
                        final double mtbf = reliabilityCalculator.calculateMTBF();
                        final double mttr = reliabilityCalculator.calculateMTTR();
                        final double availability = reliabilityCalculator.calculateAvailability();
                        ...
                        Navigator.push(
                          ...
                          MaterialPageRoute(
                            builder: (context) => ResultScreen(
                              ...
                              operationalTime: double.parse(operationalTime.toStringAsFixed(2)),
                              maintenanceTime: double.parse(maintenanceTime.toStringAsFixed(2)),
                              totalRepairs: repairs,
                            ),
                          ),
                        );
